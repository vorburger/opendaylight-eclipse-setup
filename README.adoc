...
Why do I still see red compilation errors for YANG generated code that could not be found, even though I did `mvn -Pq generate-sources` on the CLI?::
  Since the introduction of the `target-ide/` folder to completely separate the CLI and the Eclipse IDE build (required because the two would step onto each others feet, look JARs, etc. so better to keep them separate), when you do e.g. a `mvn -Pq generate-sources` (the `-Pq` is for the "quick" profile and skips a bunch of things to make your local build faster) it will generate code in `target/generated-sources`, but Eclipse will look in `target-ide/generated-sources`. Use `mvn -Pq,ide generate-sources` to activate the Maven "ide" profile to generate code into `target-ide/` instead of `target/`.
...
Do I still have to do at least `mvn -Pq,ide generate-sources` or even a full `mvn -Pq,ide install` to make red ink go away after I switch branches?::
